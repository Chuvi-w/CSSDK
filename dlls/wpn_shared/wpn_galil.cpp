...
BOOL CGalil::Deploy(void)
{
	...
	return DefaultDeploy("models/v_galil.mdl", "models/p_galil.mdl", GALIL_DRAW, "galil", UseDecrement() != FALSE);
}
...
void CGalil::GalilFire(float flSpread, float flCycleTime, BOOL fUseAutoAim)
{
	...
	Vector vecDir = m_pPlayer->FireBullets3(m_pPlayer->GetGunPosition(), gpGlobals->v_forward, flSpread,
		GALIL_DISTANCE, GALIL_PENETRATION, BULLET_PLAYER_556MM, GALIL_DAMAGE, GALIL_RANGE_MODIFER, m_pPlayer->pev, FALSE, m_pPlayer->random_seed);
	...
	PLAYBACK_EVENT_FULL(flags, m_pPlayer->edict(), m_usFireGalil, 0, (float*)&g_vecZero, (float*)&g_vecZero, vecDir.x, vecDir.y,
