...
void CFamas::PrimaryAttack(void)
{
	...
	BOOL fireBurst = FBitSet(m_iWeaponState, WPNSTATE_FAMAS_BURST_MODE) ? TRUE : FALSE;

	if (!FBitSet(m_pPlayer->pev->flags, FL_ONGROUND))
	{
		FamasFire(0.030 + 0.3 * m_flAccuracy, 0.0825, FALSE, fireBurst);
	}
	else if (m_pPlayer->pev->velocity.Length2D() > 140)
	{
		FamasFire(0.030 + 0.07 * m_flAccuracy, 0.0825, FALSE, fireBurst);
	}
	else
	{
		FamasFire(0.02 * m_flAccuracy, 0.0825, FALSE, fireBurst);
	}
}

void CFamas::SecondaryAttack(void)
{
	if (FBitSet(m_iWeaponState, WPNSTATE_FAMAS_BURST_MODE))
	{
		ClientPrint(m_pPlayer->pev, HUD_PRINTCENTER, "#Switch_To_FullAuto");
		ClearBits(m_iWeaponState, WPNSTATE_FAMAS_BURST_MODE);
	}
	else
	{
		ClientPrint(m_pPlayer->pev, HUD_PRINTCENTER, "#Switch_To_BurstFire");
		SetBits(m_iWeaponState, WPNSTATE_FAMAS_BURST_MODE);
	}
...
void CFamas::FamasFire(float flSpread, float flCycleTime, BOOL fUseAutoAim, BOOL bFireBurst)
{
	...
	Vector vecDir = m_pPlayer->FireBullets3(vecSrc, gpGlobals->v_forward, flSpread,
		FAMAS_DISTANCE, FAMAS_PENETRATION, BULLET_PLAYER_556MM, bFireBurst ? FAMAS_DAMAGE_BURST : FAMAS_DAMAGE,
		FAMAS_RANGE_MODIFER, m_pPlayer->pev, FALSE, m_pPlayer->random_seed);
	...
	PLAYBACK_EVENT_FULL(flags, m_pPlayer->edict(), m_usFireFamas, 0, (float*)&g_vecZero, (float*)&g_vecZero, vecDir.x, vecDir.y,
