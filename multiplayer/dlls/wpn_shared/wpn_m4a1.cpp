...
BOOL CM4A1::Deploy( void )
{
    ...
    if( FBitSet( m_iWeaponState, WPNSTATE_M4A1_SILENCED ) )
...
void CM4A1::SecondaryAttack( void )
{
    if( FBitSet( m_iWeaponState, WPNSTATE_M4A1_SILENCED ) )
    {
        ClearBits( m_iWeaponState, WPNSTATE_M4A1_SILENCED );
    ...
    {
        SetBits( m_iWeaponState, WPNSTATE_M4A1_SILENCED );
...
void CM4A1::M4A1Fire( float flSpread, float flCycleTime, BOOL fUseAutoAim )
{
    ...
    if( !FBitSet( m_iWeaponState, WPNSTATE_M4A1_SILENCED ) )
    {
        vecDir = m_pPlayer->FireBullets3( vecSrc, gpGlobals->v_forward, flSpread, 
            M4A1_DISTANCE, M4A1_PENETRATION, BULLET_PLAYER_556MM, M4A1_DAMAGE, M4A1_RANGE_MODIFER, m_pPlayer->pev, FALSE, m_pPlayer->random_seed );
    }
    else
    {
        vecDir = m_pPlayer->FireBullets3( vecSrc, gpGlobals->v_forward, flSpread, 
            M4A1_DISTANCE, M4A1_PENETRATION, BULLET_PLAYER_556MM, M4A1_DAMAGE_SIL, M4A1_RANGE_MODIFER_SIL, m_pPlayer->pev, FALSE, m_pPlayer->random_seed );
    ...
    PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usFireM4A1, 0, ( float* )&g_vecZero, ( float* )&g_vecZero, vecDir.x, vecDir.y, 
        ( int )( m_pPlayer->pev->punchangle.x * 100 ), ( int )( m_pPlayer->pev->punchangle.y * 100 ), 
        ( FBitSet( m_iWeaponState, WPNSTATE_M4A1_SILENCED ) ) ? true : false, FALSE );
...
void CM4A1::Reload( void )
{
    ...
    if( DefaultReload(M4A1_MAX_CLIP, FBitSet( m_iWeaponState, WPNSTATE_M4A1_SILENCED ) ? M4A1_RELOAD : M4A1_UNSIL_RELOAD, M4A1_RELOAD_TIME ) )
    {
...
void CM4A1::WeaponIdle( void )
{
    ...
    SendWeaponAnim( FBitSet( m_iWeaponState, WPNSTATE_M4A1_SILENCED ) ? M4A1_IDLE : M4A1_UNSIL_IDLE, UseDecrement() != FALSE );
}
