...
void CKnife::WeaponAnimation( int iAnimation )
{
    ...
    PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usKnife, 0, ( float* )&g_vecZero, ( float* )&g_vecZero, 0, 0, iAnimation, 2, 3, 4 );
}
...
int CKnife::Swing(int fFirst)
{
    ...
    else
    {
        ...
        if( fHitWorld )
        {
            ...
            EMIT_SOUND_DYN( m_pPlayer->edict(), CHAN_ITEM, "weapons/knife_hitwall1.wav", VOL_NORM, ATTN_NORM, 0, 98 + RANDOM_LONG( 0, 3 ) );
        }
...
void FindHullIntersection( const Vector &vecSrc, TraceResult &tr, float *pflMins, float *pfkMaxs, edict_t *pEntity )
{
    ...
    UTIL_TraceLine( vecSrc, vecHullEnd, dont_ignore_monsters, pEntity, &trTemp );
    ...
    for( int i = 0; i < 2; i++ )
    {
        for( int j = 0; j < 2; j++ )
        {
            for( int k = 0; k < 2; k++ )
            {
                ...
                UTIL_TraceLine( vecSrc, vecEnd, dont_ignore_monsters, pEntity, &trTemp );
